...
## Chile's New Age Data ("Validacion") but higher Natural Mortality
```{r}
mods2compare <- c("1.01","1.02","1.03")
```
...
* Sample sizes remained the same
* Current year is downweighted, as usual
* Natural Mortality updated to **0.45** (per the data workshop)
* Maturity at age was not provided, so the values for the FarNorth stock under the two-stock hypothesis were used.
...
## Addressing CPUE Uncertainty

With the revised Chilean age data that used the "antiguo" method, but the final year of each CPUE index was downweighted, doubling the CV from 0.2 to 0.4.
...
#### Biomass
```{r}
cpue.dw <- compareModels(geth(mods2compare,"h1"))

plot(cpue.dw,combine=T,what="biomass",stack=F,main="Biomass")
```

#### Recruitment
```{r}
plot(cpue.dw,combine=T,what="recruitment",stack=F,main="Recruitment")
```

#### Fishing Mortality
```{r }
plot(cpue.dw,combine=T,what="ftot",stack=F,main="Total Fishing Mortality")
```

### Two-stock hypothesis {.tabset}
#### Biomass
```{r}
cpue.dw <- compareModels(geth(mods2compare,"h2"))
...
```{r}
mods2compare <- c("1.04","1.05")
```

### Single-stock hypothesis {.tabset}
#### Biomass
```{r}
wtatage <- compareModels(geth(mods2compare,"h1"))

plot(wtatage,combine=T,what="biomass",stack=F,main="Biomass")
```

#### Recruitment
```{r}
plot(wtatage,combine=T,what="recruitment",stack=F,main="Recruitment")
```

#### Fishing Mortality
```{r }
plot(wtatage,combine=T,what="ftot",stack=F,main="Total Fishing Mortality")
```

### Two-stock hypothesis {.tabset}
#### Biomass
```{r}
wtatage <- compareModels(geth(mods2compare,"h2"))
...
# Final Model
Model `r FinModName` was selected as the final model for `r format(Sys.time(), "%Y")`. This model incorporates:
...
#### SSB 
```{r}
plot(h1.diag, var = "ssbPrediction")
```

#### Catch
```{r}
plot(h1.diag, var = "catchPrediction")
```
...
Note that BMSY is set to the interim level of 5500 kt in this scenario.

### Kobe Plot
```{r}
kobe(fixed_bmsy(h1.mod))
```
...
#### SSB
```{r}
plot(h2.diag, var = "ssbPrediction")
```

#### Catch
```{r}
plot(h2.diag, var = "catchPrediction")
```
...
## Likelihood Table
```{r results="asis"}
LL <- cbind(summary(h1.mod)$like,summary(h2.mod)$like)
