...
1. `R/make_2_00_dat.R`
2. `R/make_h1_2_00_ctl.R`
3. `R/run_jjm_model.R`
4. `R/run_adnuts_mcmc.R`

## Typical Workflow

For the current reduced-index sensitivity, the intended sequence is:

1. Build the reduced data file with `R/make_2_00_dat.R`.
2. Build the matching control file with `R/make_h1_2_00_ctl.R`.
3. Fit the model with `R/run_jjm_model.R`.
4. Run short `adnuts` MCMC benchmarks with `R/run_adnuts_mcmc.R`.
...
This script reads `input/1.14.dat` through the local JJM reader/writer path in `jjmR`, removes three abundance indices by name, trims the retained `Chile_AcousN` series to years 2000 and later, and writes the result as `input/2.00.dat`.
...
```bash
Rscript R/make_2_00_dat.R
```

### Custom Usage

```bash
Rscript R/make_2_00_dat.R input/1.14.dat input/2.00.dat "Chile_AcousCS,DEPM,Peru_Acoustic"
```
...
- a rewritten JJM data file, usually `input/2.00.dat`
...
## `R/make_h1_2_00_ctl.R`

### What It Does

This script reads the fitted `h1_1.14` model objects, reads `input/2.00.dat`, and writes a matching control file `config/h1_2.00.ctl`.
...
```bash
Rscript R/make_h1_2_00_ctl.R
```

### Inputs

- source control file, default `config/h1_1.14.ctl`
- source input directory, default `input`
- source results directory, default `results`
- target data file, default `input/2.00.dat`
- target control file, default `config/h1_2.00.ctl`

### Output

- a rewritten control file, usually `config/h1_2.00.ctl`

### Notes

- This script depends on `results/h1_1.14.*` being present because it uses `readJJM()` on the fitted base model.
...
- It assumes the control file name matches the model name, for example `h1_2.00` with `config/h1_2.00.ctl`.
...
```bash
Rscript R/run_adnuts_mcmc.R h1_2.00 --iter=2000 --warmup=1000 --chains=6 --cores=6
```
...
```bash
Rscript R/run_adnuts_mcmc.R h1_2.00 --dry-run=true
```
...
- The script assumes the model has already been fitted and that matching `.par` and `.cor` files exist in `results/`.
...
1. Keep `README.md` focused on model history and repository-level context.
