...
# mod0.13<- runit("mod0.13",pdf=TRUE,portrait=F,est=TRUE,exec="../src/jjms")
# mod0.14<- runit("mod0.14",pdf=TRUE,portrait=F,est=TRUE,exec="../src/jjms")
# mod0.15<- runit("mod0.15",pdf=TRUE,portrait=F,est=TRUE,exec="../src/jjms")
# mod0.16<- runit("mod0.16",pdf=TRUE,portrait=F,est=TRUE,exec="../src/jjms")
...
mod1.00
modhl[[1]]$output$Stock_1$Catch_fut_3  
modls[[1]]$output$Stock_1$Catch_fut_1  
...
modls[[1]]$output$Stock_1$Catch_fut_2  
library(PBSadmb)
library(tidyverse)
library(ggthemes)
# Substitute in adjusted N-age-2019 results to modls$outputo
mn   <- c("hl","ll","hs","ls")
cn   <- c("SQ F","C=780kt","Fmsy","C=1150kt","No fishing")
i=1
j=2
ssb.df <- data.frame()
cat.df <- data.frame()
for (j in c(1,2,3,4,5)) {
  for (i in 1:4) {
    dfn <-paste0("m_",mn[i])  
    assign(dfn,readList(paste0("results/mod1.00.",mn[i],"_1_R.rep")))    
    m <- get(dfn)
    tmp1.df <- data.frame(Fishing=cn[j],Scenario=mn[i], get(dfn)[grep(paste0("SSB_fut_",j),names(m))])
    tmp2.df <-  data.frame(Fishing=cn[j],Scenario=mn[i],get(dfn)$SSB)
    names(tmp1.df) <- names(tmp2.df) <- c("Fishing","Scenario","Year","SSB", "SD","LB","UB")
    ssb.df <- rbind(ssb.df,tmp2.df,tmp1.df)
    cat <- data.frame(Fishing=cn[j],Scenario=mn[i],Year=m$Yr,Catch=m$Obs_catch_1+m$Obs_catch_2+ m$Obs_catch_3 + m$Obs_catch_4                    )
    tmp.df <- data.frame(Fishing=cn[j],Scenario=mn[i],m[grep(paste0("Catch_fut_",j),names(m))])
    names(tmp.df) <- c("Fishing","Scenario","Year","Catch")
    cat.df <- rbind(tmp.df,cat.df,cat)
  }
}
ssb.df %>% filter(Scenario=="xl"|Scenario=="ls",Fishing!='No fishing',Fishing!='SQ F',Year>2000) %>% ggplot(aes(x=Year,y=SSB,ymin=LB,ymax=UB,col=Fishing,fill=Fishing)) + theme_few(base_size=20) + geom_ribbon(alpha=0.2,col="grey") + geom_line(aes(y=SSB,x=Year,col=Fishing),stat="identity") #+ ylim(c(0,8000))
1Gt.df %>% filter(Scenario=="hl"|Scenario=="ls") %>% ggplot(aes(x=Year,y=Catch,color=Scenario)) + theme_few() + geom_line(stat="identity") #+ ylim(c(0,8000))
cat.df %>% filter(Scenario=="xl"|Scenario=="ls",Year>2000) %>% ggplot(aes(x=Year,y=Catch,color=Fishing)) + theme_few(base_size = 20) + geom_line(stat="identity",size=2) #+ ylim(c(0,8000))
cat.df %>% filter(Year==2020)
ssb.df %>% filter(Year==2039,Scenario=='ls')
ind <- read.csv("indices.csv")
ind <- ind %>% filter(Year>1980)
cat <- read.csv("Catch.csv")
cat <- cat %>% filter(Year>1980)
library(ggpubr)
c.df <- data.frame(Year=cat$Year,SSB=ind$SSB[1:39],Catch=cat$Catch)
lm_eqn <- function(c.df){
    m <- lm(Catch ~ SSB, df);
...
c.df %>% filter(Year>2012) %>% ggplot(aes(x=SSB,y=Catch,label=Year)) + theme_few(base_size = 18) + geom_text(size=6) + stat_smooth(method='lm') +
  stat_regline_equation(label.x = 2500, label.y = 550, aes(label =  paste(..eq.label.., ..adj.rr.label.., sep = "~~~~")),size=10)+
library(GGally)
ind %>% ggpairs(ind)
ind %>% select(SSB,sc_at,n_at,sc_u, pe_at, pe_u, off_u) %>% ggcorr()
ind %>% select(SSB,sc_at,n_at,sc_u, pe_at, pe_u, off_u) %>% ggpairs()
names(ind)
ind[2:10] %>% pivot_longer(1:8) %>% filter(name!='depm',name!='China_u',!is.na(value))  %>% ggplot(aes(x=SSB,y=value,col=name)) + theme_few(base_size = 18) + geom_point(size=3) + xlim(c(0,14000)) + geom_smooth() + facet_grid(name~.)
ind[1:10] %>% pivot_longer(2:9) %>% filter(name!='pe_at',name!='sc_at',name!='depm',name!='China_u',!is.na(value))  %>% ggplot(aes(x=SSB,y=value,col=name,label=Year)) + theme_few(base_size = 18) + geom_text() + xlim(c(0,14000)) + geom_smooth(method='lm') + facet_grid(name~.) + ylab("Index")
mutate(n_at=n_at/mean(n_at), off_u=off_u/mean(off_u), pe_at=pe_at/mean(pe_at), pe_u=pe_u/mean(pe_u),sc_at=sc_at/mean(sc_at), sc_u=sc_u/mean(sc_u),     depm=depm/mean(depm) ) %>% 
ind %>% mutate(na.rm=TRUE, n_at=n_at/mean(n_at), off_u=off_u/mean(off_u), pe_at=pe_at/mean(pe_at), pe_u=pe_u/mean(pe_u),sc_at=sc_at/mean(sc_at), sc_u=sc_u/mean(sc_u),  depm=depm/mean(depm) ) #%>% ggplot(aes(x=SSB,y=sc_u)) + theme_few(base_size = 18) + geom_point(size=3) + xlim(c(0,14000)) + geom_smooth()

summary(ssb.df)
m_ls <- readList("results/For_R_1.rep")    
getwd()
modls_adj <- modls
