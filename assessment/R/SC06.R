...
library(ggplot2)
source("R/prelims.R")
ggplot(mdf,aes(x=Year,y=SSB,fill=Scenario,color=Scenario)) + 
  xlim(2010,2025) + ylim(0,15000) + 
  geom_line() + 
...
# CompMods <- combineModels(mod1.4,mod1.5)
# plot(CompMods,combine=T,what="ssb",stack=F,main="SSB")
