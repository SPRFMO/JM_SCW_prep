...
file_input <- "data/SC10_2022assessmentInputData.xlsx" # From https://southpacificrfmo.sharepoint.com/:x:/r/sites/SPRFMOSCJackMackerelWorkingGroup/Shared%20Documents/Data%20repository/SC10_2022assessmentInputData.xlsx?d=w373d989fad0b492f8c80c637cbe6b437&csf=1&web=1&e=BP0PUM
file_catch <- "data/SC10_catchByFleet.csv" # From https://southpacificrfmo.sharepoint.com/:x:/r/sites/SPRFMOSC10/Shared%20Documents/Jack%20Mackerel/01%20Meeting%20Documents/SC10-JM01_rev1%20Annex1%20CJM%20catch%20history%20data.xlsx?d=w99cc1dc8a7d24120bcaabef504f9e1c9&csf=1&web=1&e=i9w0bX
...
dat_lencomp_f <- read_excel(file_input, sheet=3) %>%
                  pivot_longer(contains("F", ignore.case=F), "fleet") %>%
...
dat_cpue <- read_excel(file_input, sheet=4) %>%
              pivot_longer(contains("F"), "fleet", values_to="index") %>%
              mutate(fleet=as.numeric(str_extract(fleet,"\\d")),
                      err=CV_CPUE * index)
...
# mod0.05 <- runit("h1_0.05",pdf=TRUE,portrait=F,est=TRUE,exec="../src/jjms")
# mod0.05 <- runit("h2_0.05",pdf=TRUE,portrait=F,est=TRUE,exec="../src/jjms")
mod_prev <- mod_new

#----------
# 0.06 Add this year's catch projections
#----------
mod_new <- mod_prev
...
#----------
# 0.07 Add current year's fishery length and age comps
#----------
mod_new <- readJJM("h1_0.06", path = "config", input = "input")
mod_prev <- mod_new
...
# mod0.07 <- runit("h1_0.07",pdf=TRUE,portrait=F,est=TRUE,exec="../src/jjms")
# mod0.07 <- runit("h2_0.07",pdf=TRUE,portrait=F,est=TRUE,exec="../src/jjms")
mod_prev <- mod_new


#----------
# 0.08 Current year's fishery (and CPUE) wtatage
#----------
mod_new <- mod_prev
cpue_ind <- grep("CPUE",mod_new[[1]]$data$Inames)
...
# mod0.08 <- runit("h1_0.08",pdf=TRUE,portrait=F,est=TRUE,exec="../src/jjms")
# mod0.08 <- runit("h2_0.08",pdf=TRUE,portrait=F,est=TRUE,exec="../src/jjms")
mod_prev <- mod_new
...
# mod0.09 <- runit("h1_0.09",pdf=TRUE,portrait=F,est=TRUE,exec="../src/jjms")
# mod0.09 <- runit("h2_0.09",pdf=TRUE,portrait=F,est=TRUE,exec="../src/jjms")
mod_prev <- mod_new
...
# mod0.10 <- runit("h1_0.10",pdf=TRUE,portrait=F,est=TRUE,exec="../src/jjms")
# mod0.10 <- runit("h2_0.10",pdf=TRUE,portrait=F,est=TRUE,exec="../src/jjms")
mod_prev <- mod_new
