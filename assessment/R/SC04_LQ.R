...
plot(diagnostics(mod0.1))

runJJM("mod0.2", path="config", input="input", exec="jjms")
mod0.1 <- readJJM("mod0.1", path="config", input="input", version="2014")
...
library(jjmR)
i=0
modtmp <- readJJM("mod1.11", path="config", input="input", version="2014")

for (i in 0:9){
  # Set peel year
  # Set name for results
  mn <- paste0("m11r_",i)
  modtmp[[1]]$control$Retro  = i
  modtmp[[1]]$control$modelName = mn
  names(modtmp) = mn
  # Write new control file
  writeJJM(object = modtmp, ctlPath = file.path(getwd(),"config"), path = getwd())
  # Run model
  runJJM(mn, path="config", input="input", version="2014",exec="jjms")
}
mn
runJJM("mod1.13",input="input",version="2014",exec="jjms")


retres <- retro(modtmp, n = 9, temp = "retro", exec = "jjms")
...
dir.mod <- paste0("model_1.11_r0",i)
...
setwd(dir.main)
mn0 <- readJJM("mod1.13", path="config", input="input", version="2014")
mn1 <- readJJM("model_1.13_r01", path="config", input="input", version="2014")
mn2 <- readJJM("model_1.13_r02", path="config", input="input", version="2014")
mn3 <- readJJM("model_1.13_r03", path="config", input="input", version="2014")
mn4 <- readJJM("model_1.13_r04", path="config", input="input", version="2014")
mn5 <- readJJM("model_1.13_r05", path="config", input="input", version="2014")
mn6 <- readJJM("model_1.13_r06", path="config", input="input", version="2014")
mn7 <- readJJM("model_1.13_r07", path="config", input="input", version="2014")
mn8 <- readJJM("model_1.13_r08", path="config", input="input", version="2014")
mn9 <- readJJM("model_1.13_r09", path="config", input="input", version="2014")

mncomb <- combineModels(mn0, mn1, mn2, mn3, mn4, mn5, mn6, mn7, mn8, mn9)
plot(mncomb, what = "biomass", combine = T, stack = F)

mod0.0 <-  runit("mod0.0", est=TRUE,portrait=F,pdf=TRUE)
mod0.0 <-  runit("mod0.0", est=TRUE,portrait=F,pdf=TRUE)
mod0.1 <-  runit("mod0.1", est=TRUE,portrait=F,pdf=TRUE)
mod0.2 <-  runit("mod0.2", est=TRUE,portrait=F,pdf=TRUE)
#mod0.3 <- runit("mod0.3", est=TRUE,portrait=F,pdf=TRUE)
mod0.3 <-  runit("mod0.3", est=TRUE,portrait=F,pdf=TRUE)
mod0.4 <-  runit("mod0.4", est=TRUE,portrait=F,pdf=TRUE)
mod0.5 <-  runit("mod0.5", est=TRUE,portrait=F,pdf=TRUE)
mod0.6 <-  runit("mod0.6", est=TRUE,portrait=F,pdf=TRUE)
mod0.7 <-  runit("mod0.7", est=TRUE,portrait=F,pdf=TRUE)
mod0.8 <-  runit("mod0.8", est=TRUE,portrait=F,pdf=TRUE)
mod0.9 <-  runit("mod0.9", est=TRUE,portrait=F,pdf=TRUE)
mod0.10 <- runit("mod0.10",est=TRUE,portrait=F,pdf=TRUE)
mod0.11 <- runit("mod0.11",est=TRUE,portrait=F,pdf=TRUE)
mod0.12 <- runit("mod0.12",est=TRUE,portrait=F,pdf=TRUE)

mod1.0 <- runit("mod1.0",est=TRUE,portrait=F,pdf=TRUE)
mod1.0 <- runit("mod1.0",portrait=F,pdf=TRUE)
mod1.1 <- runit("mod1.1",est=TRUE,portrait=F,pdf=TRUE)
mod1.2 <- runit("mod1.2",est=TRUE,portrait=F,pdf=TRUE)
mod1.3 <- runit("mod1.3",est=TRUE,portrait=F,pdf=TRUE)
mod1.4 <- runit("mod1.4",est=TRUE,portrait=F,pdf=TRUE)
mod1.5 <- runit("mod1.5",est=TRUE,portrait=F,pdf=TRUE)
mod1.6 <- runit("mod1.6",est=TRUE,portrait=F,pdf=TRUE)
mod1.7 <- runit("mod1.7",est=TRUE,portrait=F,pdf=TRUE)
mod1.7 <- runit("mod1.7",portrait=F,pdf=TRUE)
mod1.8 <- runit("mod1.8",portrait=F,pdf=TRUE)
mod1.10 <- runit("mod1.10",est=TRUE,portrait=F,pdf=TRUE)
mod1.11 <- runit("mod1.11",est=TRUE,portrait=F,pdf=TRUE)
mod1.11 <- runit("mod1.11",portrait=F,pdf=TRUE)
mod1.12 <- runit("mod1.12",est=TRUE,portrait=F,pdf=TRUE)
mod1.13 <- runit("mod1.13",est=TRUE,portrait=F,pdf=TRUE)
 
 M_mods.12 <- combineModels(ModMList)
...
plot(mod10_17,combine=T,what="recruitment",stack=F)
...
ModMList = paste0("mod1.12.", 0:9)
readJJM(ModMList,path="config",input="input",exec="jjms",version="2014")
mod1.12.0
nCores = 8
cl = makeCluster(rep("localhost",times=nCores), type = "SOCK")
registerDoSNOW(cl)
runJJM(models = ModMList, path="config", input="input", version="2014",exec="jjms")
stopCluster(cl)
