...
#' TAC allocation across fleets. 
#'
#' @description Makes a nested list of Rec objects divvying up Rec0 TAC accross fleets according to allocation allo. 
#' @param Rec0 Object of class 'Rec' \linkS4class{Rec}.
#' @param allo A vector of fractions nfleets long that sums to 1, the fleet TAC allocations
#' @return A nested list of Rec objects fleet in stock \linkS4class{Rec}.
#' @author T. Carruthers
#' @export
makeMRec = function(Rec0,allo){
  ...
  for(ff in 1:length(allo)){
    ...
    Rec@TAC =  Rec0@TAC * allo[ff]                                              # allocated catch
    out[[1]][[ff]] = Rec
  }
  out
}
...
Emp <- function (x, Data, reps = 1, 
                      Inds = c(3,6),           # Indices are: (1) Chile_AcousCS, (2) Chile_AcousN, (3) Chile_CPUE, (4) DEPM, (5) Peru_Acoustic, (6) Peru_CPUE, (7) Offshore_CPUE
                      ...
                      calib_yrs = 3,           # calibration years (which historical years are used as the reference for catch / index)
                      ...
                      delta_down = c(0.01,0.3),# between 0 and 50% downward TAC change 
                      delta_up = c(0.01,0.3),  # between 0 and 50% upward TAC change 
                      resp = 0.75,             # responsiveness slightly less linear to index change
  ...
  # x = 1; Data = readRDS("C:/temp/JMdata.rds")
  # reps = 1; Inds = c(3,6); I_freq=NA; I_wt = NA; calib_yrs = 2; enp_mult = 0.3; Ind_fac = NA; TACrng = NA; delta_down = c(0.01,0.5); delta_up = c(0.01,0.5); resp = 1; curI_2_target = NA;  HCR_CP_B = c(0,0); HCR_CP_TAC=c(0,1); Mode = 1
  ...
  if(is.na(Inds[1])){ # user does not specify AddInd indices
    I_hist0 <-array(Data@Ind[x,yind],c(1,ny))
  }else{
    I_hist0 <- array(Data@AddInd[x,Inds, yind],c(length(Inds),ny))
  } 
  
  I_hist = I_hist0 / apply(I_hist0[,caliby],1,mean,na.rm=T)
  ...
  if(is.na(Ind_fac)){ # user does not specify a rate of C/I relative to today (U formulation is less dependent on age specific transient effects on cohort - selectivity )
    MSY = Data@Misc$ReferencePoints$ReferencePoints$MSY 
    VBMSY = Data@Misc$ReferencePoints$ReferencePoints$VBMSY
    UMSY = MSY / (VBMSY+MSY)
    VB = apply(Data@Misc$StockPars$VBiomass_P[,,3,],1,sum)
    Cat = Data@Cat[,LHYrInd]
    Uest = Cat / (VB + Cat)
    ...
    Ind_fac = mean(UMSY/Uest) *0.75 # to get to 75% FMSY fishing (PGY)
  }
  ...
  #out = doMRec(Rec,allo) # divide up TAC among fleets according to allo
  Rec 
