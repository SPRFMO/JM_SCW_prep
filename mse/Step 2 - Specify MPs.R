...
# cautious > aggressive
IR1A = IR2A = IR1B = IR2B = IRMP

# Level 1 is MP multiplier
formals(IR1A)$mult = formals(IR1B)$mult = 1.0
formals(IR2A)$mult = formals(IR2B)$mult = 2.0

# Level 2 is TRP on / off
formals(IR1B)$TRP = formals(IR2B)$TRP = rep(0,7) # no TRP

class(IR1A) =  class(IR2A) = class(IR1B) = class(IR2B) = "MMP"
IRMPs = c("IR1A","IR1B","IR2A","IR2B")

cat("Index Ratio MPs Loaded (IR1A, IR1B, IR2A, IR2B - 1/2 is multiplier, A/B is TRP/no TRP) \n")
