...
for(om in 1:nOM){
  ...
  MSE = ProjectMOM(Hist, CPMPs)
  saveRDS(MSE,paste0(largedir,"MSEs_Catch_Prop/MSE_",runnam[om],".rda"))
} 
...
MMSE_list <- lapply(1:nOM, function(i) {
  mmse <- readRDS(file = paste0(largedir,"MSEs_Catch_Prop/MSE_",codes[i], ".rda"))
...
obj <- Slick::NewSlick(
  name = "Pacific Jack Mackerel (demo - catch proportions)",
  nPerf = list(nD = length(PMs), # 19
               nS = length(PMs),
               nP = 2),
  nMPs = MMSE_list[[1]]@nMPs, #4
...
obj$MP$Description <- c("All Fleet 1",
                        "All Fleet 2",
                        "All Fleet 3",
                        "All Fleet 4")
...
## Save object ----
saveRDS(obj, file = "../mse/Slick/JM_Demo_Catch_Prop_2024.slick")
