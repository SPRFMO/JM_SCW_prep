...
The single-stock model was rerun with five retrospective peels. For the
two-stock model, the first four peels converged and are shown here; the fifth
peel failed under the updated 0.06 inputs with a non-finite final objective
function and an unstable Hessian, so it is excluded from the plotted
...
#| fig-cap: !expr paste0("Analytical retrospective of spawning biomass from four successful peel runs for the southern stock (top) and Far North stock (bottom), based on Model ", diag_h2nm, " under the two-stock hypothesis.")
#| fig-height: 10
op <- par(no.readonly = TRUE)
on.exit(par(op), add = TRUE)
...
#| fig-cap: !expr paste0("Analytical retrospective of recruitment from four successful peel runs for the southern stock (top) and Far North stock (bottom), based on Model ", diag_h2nm, " under the two-stock hypothesis.")
#| fig-height: 12
op <- par(no.readonly = TRUE)
on.exit(par(op), add = TRUE)
par(mfrow = c(2, 1), mar = c(3, 4, 2, 1))
...
ADNuts diagnostic chains were run for `h1_0.06` and `h2_0.06` with four chains,
1000 iterations per chain, 500 warmup iterations, unit metric,
`adapt_delta = 0.9`, and `max_treedepth = 6`. These chains are included as
sampler diagnostics and are not suitable for posterior inference. The
single-stock run retained low effective sample sizes, high Rhat values, and
divergent transitions. The two-stock run had many divergent transitions and
also retained low effective sample sizes and high Rhat values. Longer and
better tuned runs would be required before using MCMC output for inference.
...
![ADNuts pairs plot for the ten slowest-mixing parameters in Model h1_0.06 under the single-stock hypothesis. Diagonal panels show trace plots, lower panels show posterior draws, and upper panels show pairwise correlations.](images/mcmc_diagnostics/h1_0.06_pairs_slow.png){#fig-diag-adnuts-h1-pairs-slow width=100%}

![ADNuts sampler diagnostics for Model h1_0.06 under the single-stock hypothesis.](images/mcmc_diagnostics/h1_0.06_sampler_params.png){#fig-diag-adnuts-h1-sampler width=100%}

![Marginal posterior diagnostics for the slowest-mixing parameters in Model h1_0.06 under the single-stock hypothesis.](images/mcmc_diagnostics/h1_0.06_marginals_slow.png){#fig-diag-adnuts-h1-marginals width=100%}

![Posterior standard deviations compared with MLE standard errors for Model h1_0.06 under the single-stock hypothesis.](images/mcmc_diagnostics/h1_0.06_uncertainties.png){#fig-diag-adnuts-h1-uncertainties width=100%}

![ADNuts pairs plot for the ten slowest-mixing parameters in Model h2_0.06 under the two-stock hypothesis. Diagonal panels show trace plots, lower panels show posterior draws, upper panels show pairwise correlations, and green points indicate divergent transitions.](images/mcmc_diagnostics/h2_0.06_pairs_slow.png){#fig-diag-adnuts-h2-pairs-slow width=100%}

![ADNuts sampler diagnostics for Model h2_0.06 under the two-stock hypothesis.](images/mcmc_diagnostics/h2_0.06_sampler_params.png){#fig-diag-adnuts-h2-sampler width=100%}

![Marginal posterior diagnostics for the slowest-mixing parameters in Model h2_0.06 under the two-stock hypothesis.](images/mcmc_diagnostics/h2_0.06_marginals_slow.png){#fig-diag-adnuts-h2-marginals width=100%}

![Posterior standard deviations compared with MLE standard errors for Model h2_0.06 under the two-stock hypothesis.](images/mcmc_diagnostics/h2_0.06_uncertainties.png){#fig-diag-adnuts-h2-uncertainties width=100%}
