# Appendix E. 0.07 Diagnostic Figure Series

This appendix provides the diagnostic figure series for the 0.07 model set. The selection follows the requested annex figure groups and keeps the figures organized by diagnostic purpose.
...
diag_assessment_dir <- "/Users/jim/_mymods/sprfmo/jjm/assessment"
diag_config_dir <- file.path(diag_assessment_dir, "config")
diag_input_dir <- file.path(diag_assessment_dir, "input")
diag_output_dir <- file.path(diag_assessment_dir, "results")
...
if (length(diag_missing)) {
  stop(
    "Missing files required for the 0.07 diagnostic appendix:\n",
...
diag_check_indices <- function(mod, label) {
  ...
  if (!identical(observed, diag_expected_indices)) {
    stop(
      label,
      " index names do not match the expected 0.07 configuration:\n",
...
two-stock model, peels 1 and 2 failed under the updated 0.07 inputs with
non-finite final objective functions, while peels 3-5 converged. Because the
successful two-stock peels do not form a contiguous retrospective series, the
two-stock retrospective diagnostic is marked as unavailable below rather than
plotted as an incomplete series.
...
ADNuts diagnostic chains have not yet been rerun for the 0.07 model set after
the Far North length-composition sample-size update. The MCMC tables and plots
below are included only when matching 0.07 ADNuts diagnostic output is present.

```{r}
#| label: tbl-diag-adnuts-mcmc-summary
#| tbl-cap: "ADNuts MCMC diagnostic run summary for the 0.07 model pair. These short chains are intended for convergence diagnostics, not posterior inference."
#| results: asis
if (diag_mcmc_available) {
  ...
  knitr::kable(data.frame(note = "ADNuts MCMC summary is not yet available for the 0.07 model pair."))
}
```

```{r}
#| label: tbl-diag-adnuts-mcmc-slow-parameters
#| tbl-cap: "Ten slowest-mixing parameters retained from each ADNuts fit for the 0.07 model pair."
#| results: asis
if (diag_mcmc_available) {
  ...
  knitr::kable(data.frame(note = "Slowest-mixing ADNuts parameters are not yet available for the 0.07 model pair."))
}
```

```{r}
#| label: fig-diag-adnuts-mcmc-images
#| fig-cap: "ADNuts MCMC diagnostic plots for the 0.07 model pair, shown when matching diagnostic images are available."
#| echo: false
#| results: asis
if (diag_mcmc_available) {
  knitr::include_graphics(diag_mcmc_images)
} else {
  cat("ADNuts MCMC diagnostic plots are not yet available for the 0.07 model pair.")
}
```
