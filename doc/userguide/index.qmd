...
```{=html}
<div class="paper-title">The Joint Jack Mackerel (JJM) model: A user guide</div>
<div style="margin: 0 0 1.5rem 0; text-align: center;">
Mirian GERONIMO, Criscely LUJAN, Josymar TORREJON, Elmer QUISPE<br>
...
The JJM model, implemented in AD Molder Builder ([ADMB](https://www.admb-project.org/)), uses a forward projection approach and maximum likelihood estimation to solve for model parameters. The operational population dynamics model of the JJM is defined by the standard catch equation with various modifications such as those described by Fournier & Archibald (1982)[@fournier-1982], Hilborn & Walters (1992) [@Hilborn1992] and Schnute & Richards (1995)[@schnute-1995].\
...
- $Cat^k_{y,a}$ is the catch at age $a$ for fishery $k$ in the year $y$ (see section c. Catches [3.1.3](#ch: catches){reference-type="ref" reference="ch: catches"});
...
\end{equation}$$ with $\ y_1\leq y \leq y_N+1$.\
...
With Popes approximation and without Popes approximation the survival rate for the stock $s$ at age $a$ in the year $y$ is calculated as: $$\begin{equation}
...
\end{equation}$$ for all other ages, let $r_0 := y_0-m+r_a$ the first year of recruitment and $r_a$ the recruitment age. If $r_a=1$, then:

$$\begin{equation}
N^s_{y_0,a}=e^{\mu_{R,y_0-a+1}^s + \epsilon^s_{y_0-a+1}}
            \prod_{j=1}^{a-1}e^{-M^s_{y_0,j}}, \ 1<a<m.
\end{equation}$$ Otherwise if $r_a>1$:
...
- $wt\_{mat}^s_a$ is the proportion of mature females (in weight) at each age $a$ in each stock $s$ this is calculated as a product of the population weight at age ($wt\_pop^s_a$) and the maturity ($maturity^s_a$) as follows: $$\begin{equation}
...
- according to the equation ([\[eq: b01\]](#eq: b01){reference-type="ref" reference="eq: b01"}), $SSB^s_y$ is equal to virgin biomass $B^{s,1}_0$ for the years $ssb_0\leq y \leq r_0$.
...
- ${S_{fut}}^s_{y_N+1,a} = e^{-M^s_{y_N}}$, is the survival rate future for for the sixth projection scenario. The projection scenarios can be consulted in section [3.2.7](#Fproj){reference-type="ref" reference="Fproj"} Fishing mortality for projections.
...
- ${N_{fut}}^s_{y_N,1} = SRecruit(SSB^s_{y_N+1-r_a},cum\_regs(s)+yy\_sr(s,y_N)).e^{\epsilon^s_{y_N+1}}$, where $SRecruit()$ is a function of spawning biomass and the number of regimens (for this function see f. Recruitment [3.1.6](#ch: recruitment){reference-type="ref" reference="ch: recruitment"}). $\epsilon^s_{y_N+1}$, in this case, is the future annual (for the year $y_N+1$) recruitment deviation for the stock $s$.
...
### d. Calculation of virgin biomass
...
and for the rest of regimes $$\begin{equation}
 \label{eq: b0r}
...
- $R_0^{s,1}$ is the recruitment for the first regime belonging to the stock $s$. It is obtained in section [3.1.5](#ch: s-r){reference-type="ref" reference="ch: s-r"};

- $reg\_shift^s_{r-1}$ year of regime change, is input in control file, for each stock $s$ and regime number $r$.

### e. Stock-Recruitment parameters {#ch: s-r}
...
- $R^{r}_0$ is the recruitment for the regimen $r$ ($1\leq r \leq n_{regs}$). It is calculated from: $$\begin{equation}
      R^r_0=e^{log\_Rzero(r)},
  \end{equation}$$ $log\_Rzero$ is a vector parameter to be estimated and it has $n_{regs}$ elements.

- $h^{r}$ is the steepness of the regimen $r$.

- $B^{r}_0$ is the virgin biomass for the regimen $r$ when $1\leq r \leq n_{regs}$. This is obtained from $B^{r}_0:=B^{s,r_s}_0$ where $s$ is the stock to which the $r$ regime belongs and $r_s$ is the index of regime $r$ among all regimes belonging to stock $s$ (i.e. when it corresponds to the first, second, third, etc. regime of stock $s$). Finally, $B^{s,r_s}_0$ is calculated in section $d$ (equations [\[eq: b01\]](#eq: b01){reference-type="ref" reference="eq: b01"} and [\[eq: b0r\]](#eq: b0r){reference-type="ref" reference="eq: b0r"}). Analogously it follows for $R^{r}_0 := R^{s,r_s}_0$.

### f. Recruitment {#ch: recruitment}
...
For the fish growth, the JJM uses the von Bertalanffy growth model as follows: $$\begin{equation}
    \mu_{age}(g,1)=L_0(g)
\end{equation}$$ $$\begin{equation}
...
### h. Equation weight at length

$$\begin{equation}
     wt\_age\_vb(g,i) = lw\_a  \left(\mu_{age}(g,i)\right)^{lw\_b}
\end{equation}$$

- $wt\_age\_vb(g,i)$ is the weight at length $\mu_{age}(g,i)$ [3.1.7](#ch: g){reference-type="ref" reference="ch: g"}.
...
- $maturity\_vb(g,i)$ is the proportion of mature species at length $\mu_{age}(g,i)$ where $g$ and $i$ as in section g [3.1.7](#ch: g){reference-type="ref" reference="ch: g"}.

### j. Matrix age composition to length composition
...
Be the fixed integer $g$ such that $1\leq g \leq n_{growth}$. The right expression in ([\[eq: page\]](#eq: page){reference-type="ref" reference="eq: page"}) is the normalization for $X_a$ since $X_a$ is (assumed) a random variable with normal distribution with mean $\mu_a =\mu_{age}(g,a)$ and standard deviation $\sigma_{a}= sd_{age}(g) \mu_{age}(g,a)$.\
$P\_age2len_g$ is the matrix such that $Cl(1,nyears,1,n_L) = C(1,nyears,1,m) P\_age2length_g$ where $n_L$ (input) is the number of sizes considered . $Cl$ is the annual length composition matrix, and $C$ is the annual ages composition matrix. The elements of $P\_age2len_g$ are defined as: $$\begin{equation}
...
### k. Survey Predictions {#surveypred}
...
and for $i=1+npars\_rw\_q^k :$ $$\begin{equation}
...
\end{equation}$$ where $iyr=yrs\_ind(k,y)$, besides:
...
- If $use\_age\_err$=TRUE, then: $$\begin{equation}
          eac\_ind_j(k,i)=\sum_{l=1}^m \left(age\_err^j_l \dfrac{tmp\_n_l}{\sum_{j=1}^mtmp\_n_j}\right)
  \end{equation}$$

- If $use\_age\_err$=FALSE, then: $$\begin{equation}
  eac\_ind_j(k,i)=\dfrac{tmp\_n_j}{\sum_{j=1}^mtmp\_n_j}
  \end{equation}$$
...
- $SRecruit()$ is a function of spawning biomass and the number of regimens (for this function see f. Recruitment [3.1.6](#ch: recruitment){reference-type="ref" reference="ch: recruitment"}).
...
### l. Fishery Predictions {#ch: fshpred}
...
- If $use\_age\_err=TRUE$: $$\begin{equation}
...
- If $use\_age\_err=FALSE$: $$\begin{equation}
...
- If $F^k_{Se,opt}=1:$\
  ...
  =y_0$, then the rest of the years of selectivity change are taken into account being stored in $Fyrs_{Se,ch}^{k,y}$, for $2\leq y \leq Fn_{Se,ch}^k%n\_sel\_ch\_fsh(k)$. $Fyrs_{Se,ch}^{k,y}$ and $Fn_{Se,ch}^k$ are given in the control file.
  ...
  Selectivity of the fishery $k$ in the year $y_0\leq y \leq y_N$ and each age $a$ is calculated as follows. Let $A$ be the set of years of selectivity change for a given fishery $k$ ( i.e. $A= Fyrs_{Se,ch}^{k,.}$, as stated above, it includes $y_0$) and $p_i\in A$ some element of $A$ ($p_{i+1}$ would be the next year of selectivity change): $$\begin{equation}
  ...
  \end{equation*}$$ for $\ 1\leq a \leq Fn_{Se,ages}^k$.\
  Now for the remaining ages: $$\begin{equation}
  ...
  \end{equation}$$ $$\begin{equation*}
         \left. \left.\sum_{i=Fn_{Se,ages}^k+1}^{m}e^{log\_selcoffs\_fsh_{Fn_{Se,ages}^k}(k,jj_{p_i})}\right)\right),
  \end{equation*}$$ where $Fn_{Se,ages}^k\leq a \leq m$ and $p_i\leq y < p_{i+1}$.\
  Also is calculated $$\begin{equation}
  ...
  $$\begin{equation}
      ...
      \end{tabular}
  \end{equation}$$ []{#tab: phase1 label="tab: phase1"}

- If $F^k_{Se,opt}=2:$ Let $p_i$ as before and $p_i\leq y < p_{i+1}$ $$\begin{equation}
              log\_sel\_fsh_a(k,y)=-log( 1.0 + e^{(-1.sel\_slope\_fsh(k,jj_{p_i}) * ( age\_vector_a - sel50\_fsh(k,jj_{p_i})) )})
  \end{equation}$$ for $1\leq a \leq nselages\_fsh(k)$; $$\begin{equation}
              log\_sel\_fsh_a(k,y)=log\_sel\_fsh_{Fn_{Se,ages}^k}(k,y),
  \end{equation}$$ for $nselages\_fsh(k) \leq a \leq m$.
  ...
  $$\begin{equation}
      ...
      \end{tabular}
  \end{equation}$$ []{#tab: phase2 label="tab: phase2"}

- If $F^k_{Se,opt}=3:$ Let $p_i$ as before, $$\begin{equation}
  ...
  Now for the remaining ages: $$\begin{equation}
          log\_sel\_fsh_a(k,y) = log\_sel\_fsh_{Fn_{Se,ages}^k}(k,y),
  \end{equation}$$ for $Fn_{Se,ages}^k\leq a \leq m$.
  ...
  \\        $phase\_dlogist\_fsh(k)$& = & $phase\_sel\_fsh(k)$;\\
          ...
          $logsel\_slp\_in\_fshv(k)$& = &$logsel\_slp\_in\_fsh(k,1)$;\\
             $sel\_inf\_in\_fshv(k)$& =  &  $sel\_inf\_in\_fsh(k,1)$.
      \end{tabular}
  \end{equation}$$ []{#tab: phase3 label="tab: phase3"}
...
- If $I_{Se,opt}=1$: Let $B$ the set of years of selectivity change for a given index $k$ (i.e. $B= Iyrs_{Se,ch}^{k,.}$, besides $y_0\in B$) y $p_i\in B$, some element of $B$: $$\begin{equation}
      sel\_tmp_{y,a} = log\_selcoffs\_ind_a(k,jj_{p_i})
  \end{equation}$$ for $1\leq a \leq In_{Se,ages}^k$. Now for the remaining ages $$\begin{equation}
      sel\_tmp_{y,a}= log\_selcoffs\_ind_{In_{Se,ages}^k}(k,jj_{p_i}),
  \end{equation}$$ for $In_{Se,ages}^k\leq a \leq m$.\
  Now $log\_sel\_ind$ is calculated as $$\begin{equation}
  ...
  \end{equation}$$ for $1\leq a \leq m$ and $p_i\leq y < p_{i+1}$.\
  Also is calculated $$\begin{equation}
  ...
  $$\begin{equation}
      ...
      \end{tabular}
  \end{equation}$$ []{#tab: phase4 label="tab: phase4"}

- If $I_{Se,opt}=2:$ Let $p_i$ as before, $$\begin{equation}
              log\_sel\_ind_a(k,y) = - log( 1.0 + e^{(-sel\_slope\_ind(k,jj_{p_i}) . ( age\_vector_a - sel50\_ind(k,jj_{p_i})) )}).
  \end{equation}$$ for $1\leq a \leq m$ where $p_i\leq y < p_{i+1}$.
  ...
  $$\begin{equation}
      ...
      \end{tabular}
  \end{equation}$$ []{#tab: phase5 label="tab: phase5"}

- If $I_{Se,opt}=3:$ $$\begin{equation}
  ...
  \end{equation}$$ for $1\leq a \leq In_{Se,ages}^k.$\
  Now for the remaining ages $$\begin{equation}
           log\_sel\_ind_a(k,y) = log\_sel\_ind_{In_{Se,ages}^k}(k,y),
  \end{equation}$$ for $In_{Se,ages}^k\leq a \leq m$ where $p_i\leq y < p_{i+1}$.
  ...
  - $age\_vector$ is calculated as follows: $age\_vector_a=2.(a+r_a-1)$, for $1\leq a \leq m$
    ...
    $$\begin{equation}
        \begin{tabular}{c c c}
            ...
            $sel\_inf\_in\_indv(k)$& =   & $sel\_inf\_in\_ind(k,1)$.
        \end{tabular}
    \end{equation}$$ []{#tab: phase6 label="tab: phase6"}
...
- If $sel\_map(3,k) \neq k$ then $$\begin{equation}
          log\_sel\_fsh(k)=log\_sel\_fsh(sel\_map(3,k))
  \end{equation}$$ where $k$ is the fishery number and $sel\_map(3,k)$ is the element of row 3 and column $k$ of the selectivity matrix.
...
- Si $sel\_map(2,k) = 2$ y $sel\_map(3,k) \neq k-n_{fsh}$: $$\begin{equation}
          log\_sel\_ind(k-n_{fsh}) = log\_sel\_ind(sel\_map(3,k)).
  \end{equation}$$

Finally, fishery and index selectivity respectively is obtained from $$\begin{equation}
    sel\_fsh=e^{log\_sel\_fsh},
\end{equation}$$ $$\begin{equation}
    sel\_ind=e^{log\_sel\_ind}.
\end{equation}$$

## Replacement yield, Maximum Sustainable Yield, and projections

This section presents the calculations for obtaining the replacement yield for each stock using the function $repl\_ssb()$ ([3.2.2](#replssb){reference-type="ref" reference="replssb"}). It also includes the calculations for obtaining the Maximum Sustainable Yield (MSY) using the functions $yield()$ and $yld()$, described in [3.2.5](#yield){reference-type="ref" reference="yield"} and [3.2.3](#yld){reference-type="ref" reference="yld"} respectively. Additionally, the unfished abundance ($N\_NoFsh$) is presented. The projections are then developed as follows: first, in the Fishing Mortality for Projections section [3.2.7](#Fproj){reference-type="ref" reference="Fproj"}, different scenarios of the fishing mortality rate (a total of 6) are presented, and finally, in the Future Projections section [3.2.8](#futproj){reference-type="ref" reference="futproj"}, the projections of spawning biomass and catch under these scenarios are presented.
...
In the last phase of the optimization process, the model calculates the Replacement yield, which is the volume (in weight) that can be removed from a fish stock without increasing or decreasing the biomass of the stock [@glosario]. To calculate the replacement yield for each stock $s$, it uses the Newton method of 4 iterations to find the root of the function $dssb_{s}$ defined as:

$$\begin{equation}
    dssb_{s} = \dfrac{ssb2_{s} - ssb3_{s}}{df}, \ \text{with} \  df=1e^{-3}
\end{equation}$$ $dssp_{s}$ is the derivative of $dssb_{s}$ calculated as:
...
\end{equation}$$ The iteration is on:
...
Finally, for each stock s $$\begin{equation}
    repl\_F(s) = F_1^s,
\end{equation}$$
...
- $repl\_ssb(F,s)$ is a function with two arguments $F$ and stock $s$ described in [3.2.2](#replssb){reference-type="ref" reference="replssb"}.

- $SSB^s_y$ is the spawning stock biomass for each stock $s$ and year $y$ described in b. [3.1.2](#ssb){reference-type="ref" reference="ssb"}

In addition, this function (in tpl file $Get\_Replacement\_Yield$) also calculates: $$\begin{equation}
...
\end{equation}$$ $Fratio(k)$ for each fishery $k$ ($1\leq k \leq n_{fsh}$) is obtained in ([\[fratio\]](#fratio){reference-type="ref" reference="fratio"}) and $$\begin{equation}
...
- $mod\_rec(s,i)$ is the recruitment as estimated by model. It is calculated, from year $r_0$ to year $y_N$, for each stock $s$ as $$\begin{equation}
...
This function $repl\_ssb(.,.)$ also calculates $$\begin{equation}
...
### Function yld(Fratio, Ftmp, s) {#yld}
...
- The $msy\_stuff(2)$ (MSY) is calculated from:
  ...
  Note that the sum $\displaystyle\sum_{k_s}$ in equation ([\[msystuff2\]](#msystuff2){reference-type="ref" reference="msystuff2"}) is about the indexes of fisheries belonging to stock $s$.\
...
**Obs.** This $yld()$ function in [3.2.3](#yld){reference-type="ref" reference="yld"} uses the last year of observation i.e. $y_N$ but if you want to consider another year, replace $y_N$ by the required year.
...
\end{equation}$$ $dyldp_s$ is the derivative of $dyld_s$ calculated as:
...
The iterations are on: $$\begin{equation}
        F_2^s \leftarrow F_1^s + df.0.5,
\end{equation}$$
...
### Function yield (Fratio, Ftmp,s) {#yield}
...
**Obs.** This $yield()$ function in [3.2.5](#yield){reference-type="ref" reference="yield"} uses the last year of observation i.e. $y_N$ but if you want to consider another year, replace $y_N$ with the required year.\
...
\end{equation}$$ where $N^s_{y,a}$ was obtained in Numbers at age section [3.1.1](#N){reference-type="ref" reference="N"} and $SSB^s_{y}$ was obtained in b. Spawning stock biomass section [3.1.2](#ssb){reference-type="ref" reference="ssb"}.\
For $y_0\leq y \leq y_N$, the number of recruits from stock $s$ in year $y$ is assumed equal to the number of individuals of stock $s$ in year $y$ at age 1 i.e. $recruits^s_y=N^s_{y,1}$ with $y_0 \leq y \leq y_N$. This provides the remainder of the unfished abundance for year $y$ with $y_0 < y \leq y_N$ as follows: $$\begin{equation}
...
where $SRecruit()$ is a function defined in eq. ([\[srecruit\]](#srecruit){reference-type="ref" reference="srecruit"})-(48) and no fishing spawning biomass is calculated for $y_0 \leq y \leq y_N$ and $1\leq s \leq n_{stk}$ as
...
The ratio, $Sp\_Biom\_NoFishRatio^s_y$, between spawning biomass and unfished biomass for each stock $s$ and $y_0+1\leq y \leq y_N$ is obtained from $$\begin{equation}
    Sp\_Biom\_NoFishRatio^s_y=\dfrac{SSB^s_y}{SSB\_NoFish^s_y}.
\end{equation}$$ Other quantities for each stock $s$ are $$\begin{equation}
...
With the previous $Nnext$ equation ([\[next\]](#next){reference-type="ref" reference="next"}-173) is possible to obtain other quantities such as:
...
- The acceptable biological catch ($ABCBiom$) for each stock $s$: $$\begin{equation}
...
- The overfishing limit ($OFL$) for each stock $s$:
  ...
  \end{equation}$$ ($Fratio$ is in equation ([\[fratio1\]](#fratio1){reference-type="ref" reference="fratio1"})) and $$\begin{equation}
      Ztmp_a(s)=M^s_{y_0,a}+ \sum_{k_s}Fatmp_a(k_s).
  \end{equation}$$

The remaining unfished abundance (for $y_N+1\leq y\leq endyr\_fut$) will be developed in the Future projections section [3.2.8](#futproj){reference-type="ref" reference="futproj"} as it requires additional calculations.

### Fishing mortality for projections {#Fproj}
...
- Case $iscen=1$: $$\begin{equation}
          F\_fut\_tmp_a(k)=F^k_{y_N,a};
  \end{equation}$$

- Case $iscen=2$: $$\begin{equation}
          F\_fut\_tmp_a(k) = F^k_{y_N,a} 0.75;
  \end{equation}$$

- Case $iscen=3$: $$\begin{equation}
          F\_fut\_tmp_a(k) = F^{k}_{y_N,a} 1.25;
  \end{equation}$$

- Case $iscen=4$: $$\begin{equation}
          F\_fut\_tmp_a(k)=Se^k_{y_N,a} Fratio(k) Fmsy(s),
  \end{equation}$$ where $Fratio(k)$ is in equation ([\[fratio1\]](#fratio1){reference-type="ref" reference="fratio1"});

- Case $iscen=5$: $$\begin{equation}
   \label{nfut}
          F\_fut\_tmp_a(k)=f_{tmp}(k) Se^k_{y_N,a}
  \end{equation}$$

  where $f_{tmp}(k) = ftmp_2 .Fratio(k)$ with $ftmp_2 = SolveF3(y_N, {N_{fut}}_{{styr\_fut}}^{s}, hcr\_tac(s), s)$. $hcr\_tac$ is at the end of this section and ${N_{fut}}_{{styr\_fut}}^{s}$ is described in Future Projections section [3.2.8](#futproj){reference-type="ref" reference="futproj"};

- Case $iscen=6$: $$\begin{equation}
          F\_fut\_tmp_a(k) = 0.0,
  \end{equation}$$
...
\end{equation}$$ and the future survival rate $$\begin{equation}
    {S_{fut}}^s_{y,a}=e^{-{Z_{fut}}^s_{y,a}}.
\end{equation}$$

For $iscenario = 5$, it is necessary to calculate for each stock $s$ $$\begin{equation}
    hcr\_tac(s) = \sum_{k_s} hcr\_tac\_in.p\_lastyr(k_s),
\end{equation}$$ where $\displaystyle\sum_{k_s}$ refers to the sum over the fisheries belonging to the stock $s$, $hcr\_tac\_in = 10$ and\
...
For $\ styr\_fut-r_a\leq y \leq styr\_fut-1$ future spawning biomass is assumed to be equal to spawning biomass $SSB$ (equation [\[eq: ssb\]](#eq: ssb){reference-type="ref" reference="eq: ssb"}): $$\begin{equation}
    SSB\_fut^{s,iscen}_y=\sum_{a=1}^m wt\_{mat}^s_a N^s_{y,a}(S^s_{y,a})^{s_f} .
\end{equation}$$

Now for $y=styr\_fut$ $$\begin{equation}
...
From the $N_{fut}$ equations above we can infer that given a stock number $s$, ${N_{fut}}^{s,iscen}_{y,a}$ is constant for all scenarios when $y=styr\_fut$. Let us denote this constant by ${N_{fut}}_{{styr\_fut}}^{s}$ which is required for equation ([\[nfut\]](#nfut){reference-type="ref" reference="nfut"}).

Considering the above notations, for the rest of years $y$ with $styr\_fut < y \leq endyr\_fut$ the future spawning biomass is $$\begin{equation}
...
\end{equation}$$ where $mean(natmort(s))$ is the average of the elements of the vector $natmort(s)$, this vector is obtained in function of $Mest$ .
...
where $\displaystyle\sum_{k_s}$ represents the sum over all fisheries $k_s$ belonging to stock $s$. ${F_{fut}}^{k, iscen}_{y,a}$ and ${Z_{fut}}^{s,iscen}_{y,a}$ are the future fishing mortality and future total mortality respectively, they are obtained in the Fishing mortality for projections section [3.2.7](#Fproj){reference-type="ref" reference="Fproj"} for the respective $iscen$ (see equations ([\[ffut\]](#ffut){reference-type="ref" reference="ffut"}) and ([\[zfut\]](#zfut){reference-type="ref" reference="zfut"})).
...
where $obj\_fun_1$ is defined is equation ([\[objfun1\]](#objfun1){reference-type="ref" reference="objfun1"}), $obj\_fun_2$ in equation ([\[objfun2\]](#objfun2){reference-type="ref" reference="objfun2"}) and\
...
If the current optimization phase (current_phase) is greater than 3, then: $$\begin{equation}
...
- $catch\_bio\_lva$ is the catch biomass variance. It is calculated from $$\begin{equation*}
...
- $yy\_shift\_st$ is a matrix, with $n_{stk}$ rows and and each row has $n_{reg_s}$ elements (as columns), such that $$\begin{equation}
   \label{eq: yyshiftst}
          yy\_shift\_st(s,1)=r_0
  \end{equation}$$ and $$\begin{equation*}
           yy\_shift\_st(s,r) = reg\_shift^s_{r-1}, \ 2\leq r \leq n_{reg_s}
  \end{equation*}$$ where $reg\_shift$ is matrix input;

- $yy\_shift\_end$ is a matrix, with $n_{stk}$ rows and and each row has $n_{reg_s}$ elements (as columns), such that $$\begin{equation}
   \label{eq: yyshiftend}
...
\end{equation}$$ is the sum of squares of recruitment residuals, $chi$ is the matrix of recruitment residuals and its elements are defined by
...
- $styr\_rec\_est(s,r)$ is obtained for each stock $s$ with $1\leq s \leq n_{stk}$ and each regime $r$ with $1\leq r \leq n_{reg_s}$ from: $$\begin{equation*}
...
- $endyr\_rec\_est(s,r)$ is obtained for each stock $s$ ($1\leq s \leq n_{stk}$) and each regime $r$ with $1\leq r \leq n_{reg_s}$ from:
  ...
  \end{equation*}$$ where $cum\_regs(s)$ is the number of regimes accumulated up to stock $s-1$ with $cum\_regs(1)=0$.
...
Finally, for $r\in R^*_{1,s}$=$\{r: 1\leq r \leq n_{reg_s}\}$ and $j\in J=\{1\leq j\leq Nyrs\_sr^{(iregs)}-1: (yrs\_sr^{(iregs,j+1)}-yrs\_sr^{(iregs,j)}) > 1 \}$ where $iregs = cum\_regs(s)+r$: $$\begin{equation}
...
Else (if the minimization phase is not the final phase) for $1\leq s \leq n_{stk}$: $$\begin{equation}
    R\_tmp4(s,2) = \sum_{r_s}\sum_{j=1}^{Nyrs\_sr^r}(\epsilon^s_{yrs\_sr^{(r,j)}})^2,
\end{equation}$$ otherwise $R\_tmp4(s,2)=0$, where $\displaystyle\sum_{r_s}$ is the sum over all the numbers of regimens $r$ that belong at stock s.\
With the last information, the value of $rec\_like(s,2)$ is calculated for each stock $s$ as: $$\begin{equation}
    rec\_like(s,2) = R\_tmp4(s,2) + \sum_{j=styr\_rec\_est(s,1)}^{y_N}( \epsilon^s_j)^2.
\end{equation}$$ In addition when $rec\_dev\_future$ is active then: $$\begin{equation}
...
\end{equation}$$ where $1\leq s \leq n_{stk}$ and
...
- If $log\_q\_ind_k$ is active:
  ...
  - for $q\_ind$ see section Survey Predictions in [3.1.11](#surveypred){reference-type="ref" reference="surveypred"},
...
- If $log\_q\_power\_ind(k)$ is active:
  ...
  - $q\_power\_ind(k)$ is exponent of equation ([\[eq: predind\]](#eq: predind){reference-type="ref" reference="eq: predind"}) of the abundance index, it is estimated from $q\_power\_ind(k) = e^{log\_q\_power\_ind(k) }$ where $log\_q\_power\_ind$ is a parameter to be estimated for each index survey $k$,
...
- If $log\_rw\_q\_ind(k)$ is active:
  ...
  \end{equation}$$ if is not active then $post\_priors\_indq_3(k)$=0.
...
- If $Mage\_offset(r)$ is active: $$\begin{equation}
...
- If $log\_sigmar_r$ is active: $$\begin{equation}
  ...
  - $sigmar(r)$ is estimated in equation ([\[eq: sigmar\]](#eq: sigmar){reference-type="ref" reference="eq: sigmar"}),
...
- If $log\_Linf_g$ is active: $$\begin{equation}
              post\_priors(g,4) = \dfrac{(log\_Linf_g-log\_{L_{\infty}^{prior}}_g)^2}{2.(L_{\infty,cv}^{prior}(g))^2},
  \end{equation}$$ if is not active then $post\_priors(g,4)=0.$
...
- If $log\_k_g$ is active: $$\begin{equation}
...
## Penalty function for fishing mortality
...
- If $logsel\_p1\_fsh(k)$ is active:
  ...
  \end{equation}$$ where $yr_i=Fyrs_{Se,ch}^{k,i}$ and
...
- If $log\_selcoffs\_fsh(k)$ is active:\
  Let $yr_i = Fyrs_{Se,ch}^{k,i}$ for each $i$ with $1\leq i \leq Fn_{Se,ch}^k+1$ as before, then $$\begin{equation}
  ...
  - $Fcurv_{pen}^k$ is the curvature penalty for fishery, it input for each fishery $k$ when $F^k_{Se,opt}=1$, and also if $phase\_selcoff\_fsh(k)>0$ then $$\begin{equation*}
        Fcurv_{pen}^k \leftarrow \dfrac{1.} {2{(Fcurv_{pen}^k)}^2}.
    \end{equation*}$$
...
- If $logsel\_slope\_ind(k)$ is active:\
  Let $yr_i = Iyrs_{Se,ch}^{k,i}$ for each $1\leq i \leq In_{Se,ch}^k+1$, then $$\begin{equation}
...
- If $log\_selcoffs\_ind(k)$ is active:\
  Let $yr_i = Iyrs_{Se,ch}^{k,i}$ for each $i$ with $1\leq i \leq In_{Se,ch}^k+1$ as before, then $$\begin{equation}
  ...
  Let for each $j$ such that $seldecage \leq j \leq In_{Se,ages}^k$ $$\begin{equation}
  ...
  \end{equation}$$ where $M=\{j\in\mathbb{N}: \ difftmp_j^{k,i}>0, \ seldecage \leq j \leq In_{Se,ages}^k\}$, and

  - $Icurv_{pen}^k$ is the curvature penalty for index survey, it is input for $I_{Se,opt}^k$=1, and also if $phase\_selcoff\_ind(k)>0$ then $$\begin{equation*}
        Icurv_{pen}^k \leftarrow \dfrac{1.} {{2.(Icurv_{pen}^k)}^2},
    \end{equation*}$$ $\leftarrow$ represents the new value assigned.
...
\end{equation}$$ where $act\_selcoffs\_ind=\{1\leq k \leq  n_{ind}: \ log\_selcoffs\_ind(k)\  is \ active\}$ ($obj\_fun_2$ = 0 if $act\_selcoffs\_ind = \emptyset$) and $avgsel\_ind(k,i)$ is obtained in m. Selectivity section [3.1.13](#ch: selec){reference-type="ref" reference="ch: selec"} and is only calculated when $I^k_{Se,opt}=1$.
...
- $offset\_fsh(k)$ for each fishery $k$ is calculated as follows $$\begin{equation*}
  ...
  \end{equation*}$$ where $F\_oac$ is the observed age composition matrix from fishery, its elements are denoted by $F\_oac^{k,i}_j$ for each fishery $k$, year $i$ and age $j$. $F\_oac$ is matrix input in data file.

- $eac\_fsh$ is the expected age composition matrix from fisheries calculated in section L [3.1.12](#ch: fshpred){reference-type="ref" reference="ch: fshpred"} (Fishery Predictions).
...
- $offset\_lfsh(k)$ for each fishery $k$ is calculated as follows $$\begin{equation*}
  ...
  \end{equation*}$$ where $F\_olc$ is the observed length composition matrix from fishery, its elements are denoted by $F\_olc^{k,i}_l$ for each fishery $k$, year $i$ and length group $l$. It is input in data file.

- $elc\_fsh$ is the expected length composition matrix from fisheries, and is calculated in section L [3.1.12](#ch: fshpred){reference-type="ref" reference="ch: fshpred"} (Fishery Predictions).

Now, for the survey index, let the index number $k$ ($1\leq k \leq n_{ind}$): $$\begin{equation}
...
- $offset\_lind(k)$ for each fishery $k$ is calculated as follows $$\begin{equation*}
  ...
  \end{equation*}$$ where $I\_olc$ is the observed length composition matrix from the index, its elements are denoted by $I\_olc^{k,i}_l$ for each index $k$, year $i$ and length group $l$. $I\_olc$ is matrix input in data file.

- $elc\_ind$ is the expected length composition matrix from index, and is calculated in section k [3.1.11](#surveypred){reference-type="ref" reference="surveypred"} (Survey Predictions).
...
- $offset\_ind(k)$ for each index $k$ is calculated as follows $$\begin{equation*}
  ...
  \end{equation*}$$ where $I\_oac$ is the observed age composition matrix from index, its elements are denoted by $I\_oac^{k,i}_j$ for each index $k$, year $i$ and age $j$. $I\_oac$ is input in data file.

- $eac\_ind$ is the expected age composition matrix from index, and is calculated in section k [3.1.11](#surveypred){reference-type="ref" reference="surveypred"} ( Survey predictions).
...
In the core of the model (JJM ADMB template), the parameters that would be estimated are declared using the init_prefix, optional bounds, and an optimization phase. For example in the declaration of the following bounded vector $theta$ $$\begin{equation*}
    init\_bounded\_vector \ \ theta(1,5,0.0,1.0,2),
\end{equation*}$$ the first two arguments of $theta$ (i.e. 1 and 5), are the valid indices for the parameter vector theta; the third and fourth arguments (i.e. 0.0 and 1.0) specified the bounded interval of the elements of the vector theta, and the final argument (i.e. 2) is the estimation phase.
...
Initialization values of estimable parameters of the JJM model are detailed in Table [1](#table:1){reference-type="ref" reference="table:1"}.
...
::: {#table:1}
  ...
  : Estimable parameters with their respective initialization values. In the first column are the parameters to be estimated, in the second column the names of initialization values and in the third column the equations for obtaining these values. For inputs you can consult the section [6.2.2](#ch:controlfile){reference-type="ref" reference="ch:controlfile"} (Control file).
:::
...
For $yy\_shift\_st$ and $yy\_shift\_end$ see in equation ([\[eq: yyshiftst\]](#eq: yyshiftst){reference-type="ref" reference="eq: yyshiftst"}) and ([\[eq: yyshiftend\]](#eq: yyshiftend){reference-type="ref" reference="eq: yyshiftend"}), respectively.

# File organization

This section describes the installation of ADMB, as well as the input and output files. The output files are explained for both running the model with the jjmR package (section [7.3](#ch: runjjmr){reference-type="ref" reference="ch: runjjmr"}) and running it without (section [7.4](#ch: runwithoutjjmr){reference-type="ref" reference="ch: runwithoutjjmr"}).

## Installing ADMB {#ch: installadmb}
...
- Data file (extension .dat): file containing the data for fisheries and index surveys (see section [6.2.1](#ch: datafile){reference-type="ref" reference="ch: datafile"}). Located in the folder called *input*.

- Control file (extension .ctl): file containing set-up for the parameters (see section [6.2.2](#ch:controlfile){reference-type="ref" reference="ch:controlfile"}). Located in the folder called *config*.

### Data file {#ch: datafile}
...
### Output files - with jjmR package
...
The following section shows the content of each output file. You can refer to the sections Parameter index [8](#ch: parameterindex){reference-type="ref" reference="ch: parameterindex"} or the tables ([2](#tab:my_label){reference-type="ref" reference="tab:my_label"}, [3](#tab:my_label2){reference-type="ref" reference="tab:my_label2"} and [4](#tab:my_label3){reference-type="ref" reference="tab:my_label3"}) in Appendix A [9](#ch: apendicea){reference-type="ref" reference="ch: apendicea"} to find the parameter names and their meaning.
...
- $\$ q\_$: For each index $k$ and $i-th$ year with available data ($1\leq 
...
- $\$ TotF$: $F^s_{.,.}$ is printed for each stock $s$, this is defined in the equations ([\[eq: fs\]](#eq: fs){reference-type="ref" reference="eq: fs"}) y ([\[eq: fs2\]](#eq: fs2){reference-type="ref" reference="eq: fs2"}) depending on the number of stock and Popes approximation.
...
- $\$ Catch\_fut\_iscen$: for year $i$ with $styr\_fut \leq i \leq endyr\_fut$ the future catch is printed as follows: $$\begin{equation}
      i \ \ \ \ \ \ \ catch\_future^s_{iscen, i}
  \end{equation}$$ (for $iscen=6$ the future catch is equal to zero).
...
- $\$ Index\_Q\_k$: Is printed $$\begin{equation}
      q\_ind(k,.).
  \end{equation}$$
...
- $\$ Obs\_catch\_k$: Is printed $$\begin{equation*}
      {C_{obs}}_{k,.}
  \end{equation*}$$
...
- $\$ SR\_Curve\_years\_r$: For the regimen $r$ corresponding to stock $s$ is printed $$\begin{equation}
...
- $\$ Like\_Comp:$ Is printed $obj\_comps$, where $obj\_comps$ is a vector of 14 elements as follows: $$\begin{equation}
   \label{eq:obj}
       \begin{tabular}{l l l}
  ...
  $ obj\_comps(13)$ &  = & $obj\_fun - sum(obj\_comps(1,12))$\\ 
   ...
   \end{tabular}
  \end{equation}$$ $sum()$ is the sum over all elements of the vector.
...
- $\$ Sel\_Fshry\_$: For each fishery $k$ corresponding to stock $s$ is printed $$\begin{equation}
      sel\_like\_fsh(k)
  \end{equation}$$

- $\$ Survey\_Index\_$: For each index $k$ corresponding to stock $s$ is printed $$\begin{equation}
      ind\_Like(k)
  \end{equation}$$

- $\$ Age\_Survey\_$: For each index $k$ corresponding to stock $s$ is printed $$\begin{equation}
      age\_like\_ind(k).
  \end{equation}$$
...
- $\$ Rec\_Pen$: For each stock $s$ and regime $r$ with $1\leq r \leq n_{reg_s}$ is printed $$\begin{equation}
      sigmar(rec\_map(s,r)) \ \ \ \ rec\_like(s).
  \end{equation}$$

- $\$ m\_sigmar$: Is printed $$\begin{equation}
  m\_sigmar(cum\_regs(s)+1,cum\_regs(s)+n_{reg_s}).
  \end{equation}$$

- $\$ sigmar$: For each stock $s$ and regime $r$ with $1\leq r \leq n_{reg_s}$ is printed $$\begin{equation}
      sigmar(rec\_map(s,r)).
  \end{equation}$$
...
- $\$ cv\_catchbiomass$: Is printed $$\begin{equation*}
      cv\_catchbiomass
  \end{equation*}$$ inside the model is defined as $cv\_catchbiomass = 0.05$.

- $\$ Projection\_years$: Is printed $$\begin{equation*}
      nyrs_{proj}.
  \end{equation*}$$
...
- $\$ Phase\_survey\_Sel\_Coffs$: For each index survey $k$ corresponding to stock $s$ is printed $$\begin{equation*}
      phase\_selcoff\_ind(k).
  \end{equation*}$$

- $\$ Fshry\_Selages$: For each fishery $k$ corresponding to stock $s$ is printed $$\begin{equation*}
      Fn_{Se,ages}^k.
  \end{equation*}$$

- $\$ Survy\_Selages$: For each index survey $k$ corresponding to stock $s$ is printed $$\begin{equation*}
      In_{Se,ages}^k.
  \end{equation*}$$

- $\$ Phase\_for\_age\_spec\_fishery$: For each fishery $k$ corresponding to stock $s$ is printed $$\begin{equation*}
      phase\_selcoff\_fsh(k).
  \end{equation*}$$

- $\$ Phase\_for\_logistic\_fishery$: For each fishery $k$ corresponding to stock $s$ is printed $$\begin{equation*}
      phase\_logist\_fsh(k).
  \end{equation*}$$

- $\$ Phase\_for\_dble\_logistic\_fishery$: For each fishery $k$ corresponding to stock $s$ is printed $$\begin{equation*}
      phase\_dlogist\_fsh(k).
  \end{equation*}$$

- $\$ Phase\_for\_age\_spec\_survey$: For each fishery $k$ corresponding to stock $s$ is printed $$\begin{equation*}
      phase\_selcoff\_ind(k).
  \end{equation*}$$

- $\$ Phase\_for\_logistic\_survey$: For each index survey $k$ corresponding to stock $s$ is printed $$\begin{equation*}
      phase\_logist\_ind(k).
  \end{equation*}$$

- $\$ Phase\_for\_dble\_logistic\_indy$: For each index survey $k$ corresponding to stock $s$ is printed $$\begin{equation*}
...
- $\$ msy\_mt$: Let the year $i$ with $y_0 \leq i \leq y_N$ and
  ...
  \end{equation*}$$ where $s$ is the stock to which the $k$ fishery belongs. And

  $$\begin{equation*}
      sumF(s)= \dfrac{\sum_{k_s} \left(\sum_{a=1}^mF^{k_s}_{i,a}\right)}{m}.
  \end{equation*}$$ For stock $s$ is printed
...
- $\$ age2len\_$: for each $growth\_map^s_r$ with $1\leq s \leq n_{stk}$, $1\leq r \leq n_{reg_s}$ and $1\leq j \leq m$ is printed $$\begin{equation*}
...
- $\$ rec\_like\_SRR\_stock\_k$: Is printed $$\begin{equation*}
      rec\_like(k,1).
  \end{equation*}$$

- $\$ rec\_like\_pen\_stock\_k$: Is printed $$\begin{equation*}
      rec\_like(k,2).
  \end{equation*}$$

- $\$ rec\_like\_fut\_stock\_k$: Is printed $$\begin{equation*}
      rec\_like(k,3).
  \end{equation*}$$
...
- P1: Is printed $$\begin{equation*}
              sel\_p1\_fsh.
  \end{equation*}$$

- P2: Is printed $$\begin{equation*}
              sel\_p2\_fsh.
  \end{equation*}$$

- P3: Is printed $$\begin{equation*}
              sel\_p3\_fsh.
  \end{equation*}$$

- Model name.

- Estimated annual F's: The annual total $Fmort^s_{i}$ for each stock s and year i with $y_0 \leq i \leq y_N$ is printed, that is, $$\begin{equation*}
...
- Observed survey values: For each index survey $k$ and let $s$ the stock number where $k$ belongs, is printed
  ...
  where in both cases $$\begin{equation*}
...
- Pobs_length_fishery\_: For each fishery $k$ is printed
...
- Likelihood Components: $catch\_like$, $age\_like\_fsh,$ $length\_like\_fsh,$ $sel\_like\_fsh,$ $ind\_like,$ $age\_like\_ind,$ $length\_like\_ind,$ $sel\_like\_ind,$ $rec\_like,$ $fpen,$ $post\_priors\_indq,$ $post\_priors,$ $residual$, $total$: Is printed $obj\_comps$, where $obj\_comp$ is defined in [\[eq:obj\]](#eq:obj){reference-type="ref" reference="eq:obj"}.
...
- Age likelihoods for surveys: For each index, survey $k$ is printed
  ...
  ::: center
    --------------- ----- ----------------------
     Age_Survey\_#   $k$   $age\_like\_ind(k)$.
    --------------- ----- ----------------------
  :::
...
- Selectivity penalties for surveys: Is printed
  ...
  ::: center
    --------------- ----- ----------------------- ----------------------- ------------------------
     Sel_Survey\_#   $k$   $sel\_like\_ind(k,1)$   $sel\_like\_ind(k,2)$   $sel\_like\_ind(k,3)$.
    --------------- ----- ----------------------- ----------------------- ------------------------
  :::
...
- Contribution of Priors: Is printed
  ...
  ::: center
    -------- ----------- ----------- ----------- -----------
     Source   Posterior   Param_Val   Prior_Val   CV_Prior.
  ...
  ::: center
    -------------------- ------------------------- -------------------- ---------------------- --------------------------
        Q_Survey\_#k      $post\_priors\_indq(k)$      $q\_ind(k)$          $q^{prior}(k)$        $q^{prior}_{cv}(k)$
     Q_power_Survey\_#k   $post\_priors\_indq(k)$   $q\_power\_ind(k)$   $q_{pow}^{prior}(k)$   $q_{pow,cv}^{prior}(k)$.
    -------------------- ------------------------- -------------------- ---------------------- --------------------------
  :::
...
- Rec_estimated_in_styr_endyr: Is printed
  ...
  ::: center
    --------------------------------- ----- --- ----------------------------- ------------------------------
     SR_Curve_fit\_\_in_styr_endyr\_   $r$   :   $styr\_rec\_est(istk,ireg)$   $endyr\_rec\_est(istk,ireg)$
    --------------------------------- ----- --- ----------------------------- ------------------------------
  :::
...
- Profile of stock, yield, and recruitment over F: the model name and the .dat file are printed.\
  ...
  for each integer $i$ with $1\leq i \leq 500$ where $F1_i=\dfrac{2i}{500}$ and $ttt_i=yld(Fratio, F1_i, s)$ is a vector with 5 dimensions with $$\begin{equation*}
          Fratio(k)=\dfrac{\sum_{a=1}^m F^k_{y_N,a}}{sumF(s)}, \ 1\leq k \leq n_{fsh},
  \end{equation*}$$ $s$ is the stock where the fishery $k$ belongs and $sumF(s)=\sum_{k_s}\left(\sum_{a=1}^mF^{k}_{y_N,a}\right)$.

### Output Files - without jjmR package

There are additional output files compared to when running the model with the $jjmR$ package. Some of these output files (running the JJM model using the command line in section [7.4](#ch: runwithoutjjmr){reference-type="ref" reference="ch: runwithoutjjmr"}) are:

- For_R\_$i$.csv (for each stock $i$ with $1\leq i \leq n_{stk}$): csv file with predicted and observed abundance indices.

- For_R\_$i$.rep (for each stock $i$ with $1\leq i \leq n_{stk}$): output of the $Write\_R()$ function of the ADMB template.
...
## JJM exectuable {#ch: jjmexec}

After installing ADMB (section [6.1](#ch: installadmb){reference-type="ref" reference="ch: installadmb"}), you can generate the JJM executable in the folder where the model code (i.e., the template file jjm.tpl [here](https://github.com/SPRFMO/jjm/blob/main/src/jjm.tpl)) is stored by opening, in Linux OS/Windows, a terminal in that directory and then running the following command line
...
This R package provides graphics and diagnostics libraries for SPRFMO's JJM model adopted from IMARPE's jjmTools. For the repository [see here](https://github.com/SPRFMO/jjmr). It can be installed running in R console the next command lines:

       install.packages("devtools")
       devtools::install_github("SPRFMO/jjmR")

## Running JJM with jjmR package {#ch: runjjmr}
...
- runModel.R script (suggested script in [7.3.1](#ch: runscript){reference-type="ref" reference="ch: runscript"})

- a folder called \"config\" containing the *control file* (section [6.2.2](#ch:controlfile){reference-type="ref" reference="ch:controlfile"})

- a folder called \"results\", this will contain output files

- a folder called \"input\" containing the *data file* (section [6.2.1](#ch: datafile){reference-type="ref" reference="ch: datafile"})

- a folder called \"src\" containing the executable created from the JJM model (section [7.1](#ch: jjmexec){reference-type="ref" reference="ch: jjmexec"}),
...
### R script to run the model {#ch: runscript}
...
## Running JJM without jjmR in Linux terminal {#ch: runwithoutjjmr}

To run the JJM model without the jjmR package create a new directory and save the data file [6.2.1](#ch: datafile){reference-type="ref" reference="ch: datafile"}, control file [6.2.2](#ch:controlfile){reference-type="ref" reference="ch:controlfile"} and the JJM executable [7.1](#ch: jjmexec){reference-type="ref" reference="ch: jjmexec"} there, then open a terminal (with path in that directory) and run the following command line
...
# Parameter index {#ch: parameterindex}
...
- $Se^k_{y,a}$ is the selectivity for each fishery $k$, year $y$, and age $a$, estimated by the model in the section Selectivity [3.1.13](#ch: selec){reference-type="ref" reference="ch: selec"}. In the template, it is called $sel\_fsh_a(k,y)$.
...
# Appendix A: Case study description {#ch: apendicea}

In this section a case study for the two-stock hypothesis will be discussed considering as data the file: jjm/assessment/data/1.07.dat, of [this repository](https://github.com/SPRFMO/jjm) and as control file: jjm/assessment/config/h2_1.07.ctl, from the same link.\
The control and data files are briefly described in the following tables. Note that there are 5 columns, the first is the line number it occupies in the file (data or control), the second column is the names of the parameters they take in the file (data or control), the third column is the name of the parameters they take in this document, the fourth column is the name of the parameters in the template (model code, file [jjm.tpl](https://github.com/SPRFMO/jjm/blob/main/src/jjm.tpl)) and the fifth column is the description of the parameters. Some parameters take the same name in the template and in this document, in this case this is denoted by -. These tables are intended to identify the input in the the equations of this document and in the template for this particular case study.
...
<img src="graficofases2.jpg" />
<figcaption>Optimization process of the JJM (Joint Jack Mackerel) model. In green: the inputs (control and data file), in each green box the respective inputs are listed. In blue: the optimization process, in this case it is a model with 5 phases; each box in blue is an optimization phase and the respective parameters to be estimated are listed. In red: the outputs, the top box represents the outputs produced when running the model using the command line, and in the bottom box the outputs using the jjmR package.</figcaption>
</figure>

[]{#section:AppendixA label="section:AppendixA"}

# Appendix B: Assessment results of case study

<figure id="fig:mesh2" data-latex-placement="H">
<img src="stock1.png" />
<figcaption>Plot for stock 1. Times series of (a) Total biomass, (b) Spawning biomass, (c) Recruitment, and (d) Fishing mortality.</figcaption>
</figure>

<figure id="fig:mesh3" data-latex-placement="H">
<img src="stock2.png" />
<figcaption>Plot for stock 2. Times series of (a) Total biomass, (b) Spawning biomass, (c) Recruitment, and (d) Fishing mortality.</figcaption>
</figure>

[]{#section:AppendixB label="section:AppendixB"}
