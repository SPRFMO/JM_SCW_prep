...
The annual length-composition sample-size plot was digitized from the provided figure for 1972-2025. The digitized values were treated as relative sample-size weights and rescaled by multiplying by `100 / 1094 = 0.091`, so that the largest annual effective sample size equals 100. These rescaled values were inserted as the Far North fishery length-composition sample-size vector in `input/mod0.5.dat`. The scaled input vector is archived in `doc/data/farnorth_sample_sizes_scaled_for_mod0.5.csv`. 
