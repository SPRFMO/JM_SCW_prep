...
noted that geostatistical modelling of acoustic data is generally uncommon,
contrary to geostatistical standardization of CPUE. The latter time series is
...
Day 3 follow-up items included checking that the average survey month and
seasonal assignment were implemented correctly in the model runs, reviewing fits
to zero observations, and obtaining survey length frequencies as soon as
possible.
...
profound increases and decreases over the time series. Model consistency between
the SC12 and SC13 configurations was high, and it is expected that model
consistency for the new method is of similar quality. A formal retrospective
analysis is pending and will be presented at SC14.
...
be removed from the base model. A sensitivity run should be retained that uses
the old CPUE information piecemeal before 1998, to evaluate the consequence of
preserving historical contrast before the new CPUE series begins.
...
weighted (`wtd`) CV column.

```{r tbl-chile-cpue-cv-weighting}
#| tbl-cap: "Chilean South-Central CPUE uncertainty table. The 1998-2025 rows were selected for the assessment-model input, and the weighted (`wtd`) CV is the CV used for the assessment model. The CV weight is the square-root of annual coverage relative to the maximum coverage."
#| results: asis
...
if (knitr::is_html_output()) {
  chile_cpue_cv |>
    ...
    gt::tab_header(
      ...
      subtitle = "Assessment model uses 1998-2025 and the weighted CV column"
    ) |>
    gt::cols_label(
      year = "Year",
      mean_cpue = "Mean CPUE",
    ...
    gt::fmt_number(columns = c(mean_cpue, sd_cpue), decimals = 3) |>
    ...
    gt::tab_source_note(
      "CV weight = sqrt(coverage / maximum coverage); 1/CV_wt is the inverse multiplier applied to the CV."
    ) |>
...
unsuccessful. The Day 3 discussion agreed that the new Peruvian CPUE series
starts later than the historical assessment CPUE treatment and should replace the
previous Peru CPUE index rather than be appended to it in the base model. The
previous Peru CPUE index should therefore be removed from the base configuration.
...
table of annual fleet and sampling characteristics (@tbl-chile-cpue-fishing-trip-summary).
...
2010s, and then declines in the most recent retained years.

![Normalized annual catch-rate diagnostic calculated as mean catch per set divided by hold capacity and number of fishing sets. The plot omits 1994, 2006, and 2026 and shows a LOESS smoother with an approximate 95% standard-error band.](../docs/day3/normalized_catch_rate_loess.png){#fig-chile-cpue-normalized-catch-rate width=100%}
...
therefore also be in fork length. The assessment should also increase the plus
group so the Peruvian length-frequency information and growth assumptions are
handled consistently in the model.
...
The annual length-composition sample-size plot was digitized from the provided figure for 1972-2025. The digitized values were treated as relative sample-size weights and rescaled by multiplying by `200 / 1094 = 0.1828154`, so that the largest annual effective sample size equals 200. These rescaled values were inserted as the Far North fishery length-composition sample-size vector in `input/mod0.4.dat`. The scaled input vector is archived in `doc/data/farnorth_sample_sizes_scaled_for_mod0.4.csv`. The rightmost visible bar in the source image was assigned to 2025 for the model input vector.
...
For Peru acoustic information, the meeting agreed to move to the new Peruvian
acoustic indices and remove the previous Peru acoustic index from the base
configuration. The associated Peruvian length-frequency information should be
carried forward with the new acoustic treatment. No new Chilean Acoustic North
or Chilean Acoustic South-Central index treatment was selected for the base model
at this stage.
...
|---:|---|---|---|
| 1 | Use the new Chilean CPUE series beginning in 1998. | Chile CPUE | Implement in the base model and remove the previous Chilean CPUE index. |
| 2 | Apply the Chilean effort-creep treatment with 1% annual creep through 2004 and the 0.669 adjustment beginning in 2005. | Chile CPUE / effort creep | Confirm the exact implementation in the model input and control files. |
| 3 | Weight Chilean CPUE CVs by percentage coverage and include standard errors from the INLA analysis. | Chile CPUE uncertainty | Prepare the annual uncertainty vector for assessment input. |
| 4 | Retain a sensitivity with piecemeal use of the old Chilean CPUE before 1998. | Chile CPUE sensitivity | Add to the candidate sensitivity-run list. |
| 5 | Use the new Peruvian CPUE series as a later-starting replacement and remove the previous Peru CPUE index. | Peru CPUE | Implement the replacement in the base model. |
| 6 | Treat Peruvian length frequencies as fork length and use growth parameters in fork length. | Peru biology / composition | Check consistency of growth, length-frequency bins, and length-weight inputs. |
| 7 | Increase the plus group for the Peruvian length-frequency and growth treatment. | Peru biology / composition | Specify the plus-group age in the candidate model configuration. |
| 8 | Use the new Peruvian acoustic indices and remove the previous Peru acoustic index. | Peru acoustic index | Carry forward associated length-frequency information with the new acoustic treatment. |
| 9 | Retain raw/design-based Chile Acoustic North biomass for assessment use and keep spatio-temporal acoustic modelling as research and diagnostic information. | Chile acoustic indices | Acknowledge INPESCA's modelling contribution and document why model-based CPUE indicators and design-based survey indicators are treated differently. |
| 10 | Do not add a new base-model treatment for Chilean Acoustic South-Central at this stage. | Chile acoustic indices | Retain as diagnostic or supporting information unless a later sensitivity is requested. |
...
| Add report text acknowledging the INPESCA spatio-temporal Chile Acoustic North modelling contribution and explaining why model-based CPUE indicators and design-based survey indicators are treated differently | Chile acoustic contributors | Assessment/index subgroup | Before report adoption |
...
| Provide Peru acoustic survey length frequencies as soon as possible | Peru acoustic contributors | Assessment/index subgroup | Before final model-run list |
| Check average month, seasonal assignment, and fits to zero observations for the new Peru acoustic index runs | Assessment subgroup | Peru acoustic contributors | Before final model-run list |
...
| Add number and proportion of sets by year to the Chile CPUE data-summary table | Chile CPUE contributors | Assessment/index subgroup | Before final model-run list |
| Prepare the new Chilean CPUE input beginning in 1998, including the agreed effort-creep adjustment and annual CVs weighted by percentage coverage with INLA standard errors | Chile CPUE contributors | Assessment subgroup | Before final model-run list |
| Draft a paragraph explaining the criteria for starting the new Chilean CPUE series in 1998, including data coverage, representativeness, catch history, and fleet-behaviour changes after 1997 | Chile CPUE contributors | Assessment subgroup | Before report adoption |
| Define what CPUE model or data changes will be allowed between the benchmark and subsequent assessment or MSE implementation | Chair / Assessment subgroup | Members | Before final report adoption |
| Add the sensitivity using piecemeal old Chilean CPUE before 1998 | Assessment subgroup | Chile CPUE contributors | Before final model-run list |
...
| Replace the previous Peru CPUE index with the new later-starting Peruvian CPUE series | Assessment subgroup | Peru CPUE contributors | Before final model-run list |
| Replace the previous Peru acoustic index with the new Peruvian acoustic indices and associated length-frequency information | Assessment subgroup | Peru acoustic contributors | Before final model-run list |
| Prepare Peruvian length-frequency inputs in fork length, with fork-length growth parameters and the agreed increased plus group | Assessment subgroup | Peru biology contributors | Before final model-run list |
| Provide Peru composition sample-size information in terms of numbers of fish where available, in addition to sampling-event counts | Peru biology contributors | Assessment subgroup | Before biological-input finalization |
| Evaluate whether the SPRFMO length-bin range should be expanded to match Peru's 5-90 cm length bins | Assessment subgroup | Peru biology contributors | Before biological-input finalization |
...
- The Chilean effort-creep correction may be confounded with catchability changes, and the assessment consequence should be tested alongside the agreed base treatment.
- The new Chile CPUE base treatment begins in 1998; the old pre-1998 CPUE sensitivity is needed to evaluate the effect of retaining earlier historical contrast.
- The new Peru CPUE and Peru acoustic treatments remove previous assessment indices, so the implementation should document exactly which historical series have been dropped.
...
{{< include appendix_diagnostics.qmd >}}
